...
app.post('/api/feedback', upload.array('media', 5), async (req, res) => {
  try {
    ...
    // ── ⭐ Автомат тааруулга ──
    // Хэрэв "олдсон" пост нийтэгдвэл, өмнө нь "мартсан" пост нийтэлсэн
    // хүмүүсээс ангилал/текст таарвал тэдэнд мэдэгдэл илгээнэ
    if (type === 'олдсон') {
      try {
        // Mongoose query — өмнө нь "мартсан" нийтэлсэн постуудыг хайх
        const martsanPosts = await Feedback.find({
          type: 'мартсан',
          isDeleted: { $ne: true },
        });

        for (const martsan of martsanPosts) {
          if (!martsan.userId || martsan.userId === userId) continue;

          // ⭐ Автобусны чиглэлийн дугаар ЗААВАЛ таарах ёстой
          if (!busNumber || !martsan.busNumber) continue;
          if (busNumber.toLowerCase() !== martsan.busNumber.toLowerCase()) continue;

          let isMatch = false;

          // 1) Ангилал таарах
          if (category && martsan.message && 
              martsan.message.toLowerCase().includes(category.toLowerCase())) {
            isMatch = true;
          }

          // 2) Мессеж дотроос түлхүүр үг таарах (Цүнх, Түлхүүр, Утас, г.м.)
          const keywords = ['цүнх', 'түлхүүр', 'түрийвч', 'утас', 'бичиг', 
                            'хувцас', 'малгай', 'бээлий', 'цаг'];
          for (const kw of keywords) {
            if ((message || '').toLowerCase().includes(kw) && 
                (martsan.message || '').toLowerCase().includes(kw)) {
              isMatch = true;
              break;
            }
          }
          ...
          if (isMatch) {
            await Notification.create({
              userId: martsan.userId,
              fromUser: userId || '',
              fromName: userName || 'Хэрэглэгч',
              type: 'match',
              message: `${busNumber}-р чиглэлд таны мартсан зүйлтэй төстэй олдсон пост гарлаа! "${(message || '').substring(0, 50)}"`,
              postId: feedback._id,
            });
          }
        }
      } catch (matchErr) {
        console.log('Auto-match error:', matchErr.message);
      }
    }
    ...
    if (mentions && mentions.length > 0) {
      for (const mentionName of mentions) {
        try {
          ...
          if (parts.length >= 2) {
            mentionedUser = await User.findOne({ lastName: parts[0], firstName: parts[1] });
          }
          if (!mentionedUser) {
            mentionedUser = await User.findOne({ $or: [
              { firstName: mentionName.trim() },
              { lastName: mentionName.trim() },
              { firstName: new RegExp(mentionName.trim(), 'i') },
            ]});
          }
...
app.put('/api/feedback/:id/comment/:commentIndex/like', async (req, res) => {
  try {
    ...
    const comment = feedback.commentsList[idx];
      ...
      if (comment.userId && comment.userId !== userId) {
        try {
          ...
          await Notification.create({
            ...
            fromUser: userId,
            fromName: liker?.firstName || '',
            type: 'comment_like',
            message: `${liker?.firstName || 'Хэрэглэгч'} таны сэтгэгдэлд зүрх дарлаа ❤️`,
            postId: feedback._id,
          });
        } catch (_) {}
      }
    ...
    if (commentOwner && commentOwner !== userId) {
      try {
        await Notification.create({
          ...
          fromName: userName || '',
          type: 'reply',
          message: `${userName || 'Хэрэглэгч'} таны сэтгэгдэлд хариулав`,
          postId: feedback._id,
        });
      } catch (_) {}
    }
...
app.delete('/api/feedback/:id/comment/:commentIndex', async (req, res) => {
  try {
    ...
    feedback.commentsList.splice(idx, 1);
    feedback.comments = feedback.commentsList.length;
    await feedback.save();
    res.json(feedback);
  } catch (err) {
...
app.put('/api/feedback/:id/comment/:commentIndex', async (req, res) => {
  try {
    ...
    if (feedback.commentsList[idx].userId !== userId) {
      return res.status(403).json({ error: 'Зөвхөн өөрийн сэтгэгдлийг засаж болно' });
    }

    feedback.commentsList[idx].message = message;
    await feedback.save();
    res.json(feedback);
  } catch (err) {
    res.status(400).json({ error: err.message });
...
// ── GET /post/:id — Share хийсэн постыг нээх (redirect to Flutter app) ──
app.get('/post/:id', async (req, res) => {
  try {
    const feedback = await Feedback.findById(req.params.id).lean();
    if (!feedback) return res.status(404).send('Пост олдсонгүй');
    res.redirect(`http://localhost:5000/#/post/${req.params.id}`);
  } catch (err) {
    res.status(500).send('Серверийн алдаа');
  }
});

// ── GET /api/feedback/:id — Нэг пост авах ──
...
app.get('/api/users/search', async (req, res) => {
  try {
    ...
    }).select('firstName lastName').limit(5);
    res.json(users);
  } catch (err) {
    res.json([]);
  }
...
app.post('/api/auth/login', async (req, res) => {
  try {
    ...
    if (role === 'Жолооч') {
      ...
      const driver = await Driver.findOne({ driverLicense, companyCode });
      if (!driver) {
...
app.put('/api/drivers/:id', async (req, res) => {
  try {
    ...
    if (!driver) return res.status(404).json({ error: 'Олдсонгүй' });
    const { lastName, firstName, phone, password, driverLicense, companyCode, companyName, busRoute, busNumber } = req.body;
    ...
    if (companyCode) driver.companyCode = companyCode;
    if (companyName !== undefined) driver.companyName = companyName;
    if (busRoute !== undefined) driver.busRoute = busRoute;
    if (busNumber !== undefined) driver.busNumber = busNumber;
    if (password && password.length >= 6) driver.password = password;
    await driver.save();
    res.json({ message: 'Засагдлаа', driver: { ...driver.toObject(), password: undefined } });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
...
app.delete('/api/drivers/:id/permanent', async (req, res) => {
  try {
    ...
    res.json({ message: 'Бүрмөсөн устгагдлаа' });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
